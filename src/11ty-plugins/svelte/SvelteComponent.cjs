...
class EleventySvelteComponent {
	constructor(filename, options = { ssr: true }, eleventyConfig) {
		this.content = fs.readFileSync(filename, "utf8");

		this.isSSR = options?.ssr ?? true;
		this.componentsOutputDir = options?.componentsOutputDir;
		this.eleventyConfig = eleventyConfig;
		this.parsed = path.parse(filename);
		this.filename = this.componentsOutputDir
	...
	generateClientBundle() {
		...
		let target = new FileTarget(this.filename, this.eleventyConfig);
